...
<!-- Specific Page Vendor -->
   ...
   var routeassignid = $('#routeassignid').val();
	 ...
	 calculatedistanceshops(routeid,position.coords.latitude,position.coords.longitude);
	 }

		function savesalesManAssignTransaction(clat,clong,shopid){
 		var routeassignid = $('#routeassignid').val();
           alert(shopid);
			$.ajax({
			  url: "../src/savesalesManAssignTransaction",
			  method: "POST",
			  data: ({
			 	 latitude:clat,
			 	 longitude:clong,
    ...
    function calculatedistanceshops(routeassignid,clat,clong){

			$.ajax({
				url: "../src/fetchShopkeeperInfo",
				method: "POST",
				data: ({ RouteId: routeassignid}),
				dataType: 'json',
				success: function (response) {
          var shopid =[];
					var count = Object.keys(response).length;
					for (var i = 0; i < count; i++) {
						var distance = getDistanceFromLatLonInKm(clat,clong,response[i].latitude,response[i].longitude);

						if(distance<0.200){
							shopid.push(response[i].shopid);
						}
                  
          }
            // alert(shopid);
          savesalesManAssignTransaction(clat,clong,shopid);
